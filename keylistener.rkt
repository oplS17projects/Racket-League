...
(define (game-key-listener w ke)
  (cond ((key=? ke "left") (left-turn 2))
        ...
        ((key=? ke "down") (slow-car 2)) ; ((car2 'set-acc) #f)
        ((key=? ke "up") (accelerate-car 2)) ; ((car2 'set-acc) #t)
        ((key=? ke "a") (left-turn 1))
        ((key=? ke "d") (right-turn 1))
        ((key=? ke "s") (slow-car 1)) ; ((car1 'set-acc) #f)
        ((key=? ke "w") (accelerate-car 1)) ; ((car1 'set-acc) #t)
        (else "Nothing to do")))
