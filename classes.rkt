...
(define (make-car pos theta name color)
  (let* ((image (overlay/align "right" "middle" (rectangle (/ car-width 5) car-length "solid" "black")(rectangle car-width car-length "solid" color)))
    ...
    (define (update-c lst)
      ...
      (set! theta (caddr lst)))
    (define (update-t newTheta)
      (set! theta newTheta))
    (define (update-v VxVy)
      (set! velocity VxVy))
    (define (update-p XY)
      (set! pos XY))
    (define (reset-c oPos oTheta)
      (update-c (list oPos '(0 0) oTheta)))
    (define (turn-true direction)
    ...
    (define (dispatch m)
      (cond ((eq? m 'get-x) (car pos))
            ...
            ((eq? m 'update-theta) update-t)
            ((eq? m 'update-velo) update-v)
            ((eq? m 'update-pos) update-p)
